...
void tmc2209_full(tmc2209_t *stepper, uint8_t en_pin, uint8_t dir_pin, uint8_t step_pin,
                                      uint8_t rx_pin, uint8_t tx_pin,  uint8_t ms1_pin, uint8_t ms2_pin,
                                      tmc2209_adress adress);

// set the microsteps of the driver
void tmc2209_set_microsteps(tmc2209_t *stepper, tmc2209_microstep microsteps);
// get the microsteps of the driver
tmc2209_microstep tmc2209_get_microsteps(tmc2209_t *stepper);

// set the step delay between one high/low pulse in µs
void tmc2209_set_step_delay(tmc2209_t *stepper, uint32_t step_delay_us);
// get the step delay between one high/low pulse in µs
uint32_t tmc2209_get_step_delay(tmc2209_t *stepper);

// set the spin-direction of the motor
void tmc2209_set_direction(tmc2209_t *stepper, tmc2209_direction dir);
// get the spin-direction of the motor
tmc2209_direction tmc2209_get_direction(tmc2209_t *stepper);


// functions for moving the motor via STEP/DIR interface

// move the motor by steps and direction
void tmc2209_step(tmc2209_t *stepper, uint32_t steps, tmc2209_direction dir);

// move the motor by degrees, sign specifies the direction
void tmc2209_rotate(tmc2209_t *stepper, int32_t degree);

// all functions are non blocking, therefore call update function every loop iteration
void tmc2209_update(tmc2209_t *stepper);
